#!/usr/bin/env tsx
/**
 * Generates tailwindColors.ts with all Tailwind v4 colors converted from oklch to hex.
 *
 * Run: npx tsx tools/generateTailwindColors.ts
 */
...
// Tailwind v4 color palette in oklch format
// Source: https://tailwindcss.com/docs/colors
const tailwindOklchColors: Record<string, Record<string, string>> = {
  red: {
...
function oklchToHex(oklchStr: string): string {
  ...
  if (!hex) {
    throw new Error(`Failed to convert to hex: ${oklchStr}`);
  }
  return hex;
}

async function main() {
  console.log('Generating Tailwind colors...\n');

  const colors: Record<string, string> = {};
  const shades = ['50', '100', '200', '300', '400', '500', '600', '700', '800', '900', '950'];

  for (const [colorName, shadeValues] of Object.entries(tailwindOklchColors)) {
    for (const shade of shades) {
      const oklch = shadeValues[shade];
      if (oklch) {
        const key = `${colorName}.${shade}`;
        colors[key] = oklchToHex(oklch);
  ...
  for (const [name, value] of Object.entries(additionalColors)) {
    colors[name] = value;
  }

  const colorNames = Object.keys(tailwindOklchColors);

  const output = `// ==========================================================================
// Auto-generated by tools/generateTailwindColors.ts - DO NOT EDIT MANUALLY
// Run: npx tsx tools/generateTailwindColors.ts
 ...
 * All colors are converted from oklch to hex for VS Code theme compatibility.
 */
...
/**
 * All Tailwind color keys (e.g., "red.500", "blue.400", "slate.900").
 */
...
/**
 * Tailwind color names (red, orange, amber, etc.).
 */
export type TailwindColorName = ${colorNames.map((n) => `"${n}"`).join(' | ')};

/**
 * Array of all color keys for iteration.
 */
export const tailwindColorKeys = Object.keys(tailwindColors) as TailwindColor[];

/**
 * Get the hex value for a Tailwind color.
 */
export function getTailwindColor(color: TailwindColor): string {
  return tailwindColors[color];
}
`;

  const outputPath = join(__dirname, '../generated/tailwindColors.ts');
  await writeFile(outputPath, output);

  console.log(`\n✓ Generated src/tailwindColors.ts with ${Object.keys(colors).length} colors`);
}
